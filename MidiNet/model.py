...
class MidiNet(object):
    ...
    def build_model(self):
        ...
        self.d_loss_real = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits, 0.9 * tf.ones_like(self.D)))
    ...
    def train(self, config):
        ...
        if config.dataset == 'MidiNet_v1':
            ...
            data_X = np.load(
                'your_training_data.npy')  # Shape: (n, 1, 16, 128), where n is the number of measures(bars) of training data.
            prev_X = np.load(
                'your_training_data_previous_bar.npy')  # Shape: (n, 1, 16, 128), if the bar is a first bar, it's previous bar = np.zeros(1,16,128)
            ...
            for idx in xrange(0, batch_idxs):
                ...
                _, summary_str = self.sess.run([d_optim, self.d_sum],
                                               feed_dict={self.images: batch_images, self.z: batch_z,
                                                          self.y: batch_labels, self.prev_bar: prev_batch_images})
                ...
                errD_fake = self.d_loss_fake.eval(
                    {self.z: batch_z, self.y: batch_labels, self.prev_bar: prev_batch_images})
    ...
    def generator(self, z, y=None, prev_x=None):
        ...
        h2 = tf.nn.relu(
            self.g_bn2(deconv2d(h1, [self.batch_size, 4, 1, self.gf_dim * 2], k_h=2, k_w=1, d_h=2, d_w=2, name='g_h2')))
        h2 = conv_cond_concat(h2, yb)
        h2 = conv_prev_concat(h2, h2_prev)

        h3 = tf.nn.relu(
            self.g_bn3(deconv2d(h2, [self.batch_size, 8, 1, self.gf_dim * 2], k_h=2, k_w=1, d_h=2, d_w=2, name='g_h3')))
        h3 = conv_cond_concat(h3, yb)
        h3 = conv_prev_concat(h3, h1_prev)

        h4 = tf.nn.relu(self.g_bn4(
            deconv2d(h3, [self.batch_size, 16, 1, self.gf_dim * 2], k_h=2, k_w=1, d_h=2, d_w=2, name='g_h4')))
