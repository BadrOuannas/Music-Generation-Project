...
def check_melody_range(note_list_all, dur_list_all):
    ...
    for i in range(len(note_list_all)):
        ...
        # if len(song[1:]) == 0:
        #     ipdb.set_trace()
        if min(song[1:]) >= 60 and max(song[1:]) <= 83:
            in_range += 1
...
def beats_(list_):
    ...
    for file_ in list_:
        try:
            chorus_file = ET.parse(file_)
            root = chorus_file.getroot()
            beats = chorus_file.findall('.//beats_in_measure')
            num = beats[0].text
            if num == '4':
...
def main():
    ...
    if is_get_data == 1:
        ...
        note_list_all, dur_list_all = transform_note(c_key_list, d_key_list, e_key_list, f_key_list, g_key_list,
                                                     a_key_list, b_key_list)
